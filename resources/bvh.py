...
def load(filename, order=None):
    ...
    i = 0
    active = -1 # Pointer to index of last joint added
    end_site = False

    names = []
    orients = np.array([]).reshape((0, 4))
    offsets = np.array([]).reshape((0, 3))
    parents = np.array([], dtype=int) # Pointer to parent index (-1 for root)
    ...
    for line in f:
        ...
        if rmatch:
            # append default value, will be overwritten at OFFSET line
        ...
        if chanmatch:
            ...
            if order is None:
                ...
                if any([p not in channelmap for p in parts]):
                    continue
                order = "".join([channelmap[p] for p in parts]) # Convert 'Xrotation' to 'x' (y, z too)
            continue
        ...
        # Frames: seperates HIERARCHY with MOTION
        fmatch = re.match("\s*Frames:\s+(\d+)", line)
        if fmatch:
            fnum = int(fmatch.group(1)) # number of frames
            positions = offsets[np.newaxis].repeat(fnum, axis=0) # init per-frame root & joint positions
            rotations = np.zeros((fnum, len(orients), 3)) # init per-frame joint rotations with 0
            continue
        ...
        if dmatch:
            ...
            N = len(parents)
            fi = i # fi is more readable (frame index)
            if channels == 3: # channels contains last CHANNELS line
                positions[fi, 0:1] = data_block[0:3]            # root per-frame position
                rotations[fi, :] = data_block[3:].reshape(N, 3) # root & joint per-frame rotation
            elif channels == 6:
