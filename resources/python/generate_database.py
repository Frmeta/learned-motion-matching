...
test_files = [
    ...
    # small clip for experimenting
    ('resources/bvh/test/run2_subject1.bvh', 4000, 5000),
    ('resources/bvh/test/walk3_subject2.bvh', 3000, 4000), 
...
def generate_database(config):
    ...
    for entry in config['files']:
        ...
        if config['has_gait']:
            ...
            filename, start, stop = entry
            is_crouch, is_idle, is_jump = False, False, False
            rest = []
        ...
        for mirror in [False, True] if config['is_mirror'] else [False]:
    ...
    with open('resources/bin/' + config['output_bin'], 'wb') as f:
    ...
    print("Writing " + config['output_csv'] + '...')
    ...
    with open('resources/bin/' + config['output_csv'], 'w') as csv_f:
    ...
    print(f"{config['output_csv']} written successfully!")


    """ Write Range Metadata CSV (for runtime GUI labels) """
    
    if config['is_generate_range_metadata']:
...
test_config = {
    'files' : test_files,
    'output_bin' : "database_test.bin",
    'output_csv' : "database_test.csv",
